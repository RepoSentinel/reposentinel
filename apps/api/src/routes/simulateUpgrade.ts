  ...
  app.post("/simulate/upgrade", async (req, reply) => {
    ...
    if (!body?.currentLockfile?.manager || !body?.currentLockfile?.content) {
      return sendProblem(reply, req, {
        status: 400,
        title: "Bad Request",
        detail: "currentLockfile is required",
      });
    }
    ...
    const maxBytes = Number(
      process.env.SIMULATE_MAX_LOCKFILE_BYTES ?? 2_000_000,
    );
    ...
    if (
      body.proposedLockfile?.content &&
      body.proposedLockfile.content.length > maxBytes
    ) {
      return sendProblem(reply, req, {
        status: 413,
        title: "Payload Too Large",
        detail: "proposed lockfile too large",
      });
    }
